    ...
    def _render_executor_prompt(self) -> str:
        ...
        if self._settings.is_server_role:
            text += (
                ...
                "clear `task` brief and the absolute `folder` to work in. It runs "
                "an autonomous coding agent on the laptop that edits, runs tests, "
                "and commits on its own, and needs only ONE approval up front. It "
                "returns immediately with a `developer_id`; the work runs "
